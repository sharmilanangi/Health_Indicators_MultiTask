...
class MultiTaskNet(nn.Module):
    def __init__(self, embed_dim=11348, layer_sizes=[11348, 500, 11348, 500]):
        ...
        self.last_layer = nn.Linear(layer_sizes[3], 1 ) ## change if we need classification or softmax
...
writer = SummaryWriter('logdir/cmr_model')
...
for e in range(epochs):
    ...
    print(f"===========> VALIDATION EPOCH {e}, MSE LOSS - {dev_loss_per_epoch}, R2 LOSS - {dev_r2_loss_per_epoch} ")

print("TESTING THE MODEL")

test_loss_per_epoch, test_r2_loss_per_epoch =  evaluate_model(model, test_dataloader)
print(f"===========> FINAL TEST, MSE LOSS - {test_loss_per_epoch}, R2 LOSS - {test_r2_loss_per_epoch} ")
